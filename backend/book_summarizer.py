...
class BookSummarizer:
    ...
    def text_summarization(self, text, min_length=5, max_length=77):
        ...
        text_len = len(tokens)
        summary = self.summarizer(text, min_length=min(min_length, text_len),
                             max_length=min(text_len, max_length), truncation=True)
        self.summarized_paragraphs_count += 1
        return summary[0]['summary_text']

    async def summarize_book(self, input_file: Path, output_dir: Path, progress_callback: Callable[[int, int],[]] = None):
        """
        Summarizes a book given its input file and saves the 
        summarized content to the specified output directory.
        ...
        num_paragraphs_per_chapter = [len(chapter["paragraphs"]) for chapter in book["chapters"]]
        ...
        if progress_callback: progress_callback(0, total_to_process)
        for ch_num, chapter in enumerate(book["chapters"]):
            paragraph_summaries = []

            for paragraph in chapter["paragraphs"]:
                paragraph_summaries.append(
                    self.text_summarization(paragraph))
                num_processed += 1
...
if __name__ == '__main__':
    ...
    asyncio.run(Booksummarizer.summarize_book(Path(args.input_file), Path(args.output_dir), print_progress))
